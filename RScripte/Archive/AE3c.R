...
data <- read.csv("E:/Desktop/FoPra_Sim experiment3_comment-table.csv", 
                 skip = 6, 
                 header = TRUE, 
                 sep = ",")
...
# Spalten wählen
data_selected <- data_clean[, c("run_number", "commenting_prob", "step", "adopter", "non-adopter", "moderate-interestet")]
...
data_summarized <- aggregate(cbind(adopter, `non-adopter`, `moderate-interestet`) ~ commenting_prob + step, 
                             data = data_selected, 
                             FUN = mean)
...
# Diagramm erstellen
ggplot(data_filtered, aes(x = step, y = adopter, color = factor(commenting_prob), linetype = factor(commenting_prob))) +
  geom_line() +
  labs(title = "ID mit unterschiedlicher Kommentar Wahr.",
       x = "Step",
       y = "Adaptierer",
       color = "Wahr. Kommentar",
       linetype = "Wahr. Kommentar") +
  theme_minimal()
