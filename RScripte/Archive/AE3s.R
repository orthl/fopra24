...
data <- read.csv("E:/Desktop/FoPra_Sim experiment3_share-table.csv", 
                 skip = 6, 
                 header = TRUE, 
                 sep = ",")
...
# Spalten wählen
data_selected <- data_clean[, c("run_number", "sharing_prob", "step", "adopter", "non-adopter", "moderate-interestet")]
...
data_summarized <- aggregate(cbind(adopter, `non-adopter`, `moderate-interestet`) ~ sharing_prob + step, 
                             data = data_selected, 
                             FUN = mean)
...
# Diagramm erstellen
ggplot(data_filtered, aes(x = step, y = adopter, color = factor(sharing_prob), linetype = factor(sharing_prob))) +
  geom_line() +
  labs(title = "ID mit unterschiedlicher Teilen Wahr.",
       x = "Step",
       y = "Adaptierer",
       color = "Wahr. Teilen",
       linetype = "Wahr. Teilen") +
  theme_minimal()
